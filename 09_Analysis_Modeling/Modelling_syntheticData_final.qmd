...
Since we cannot predict the outcomes of the XGBoost modelling (in the previous script @ADDREFTOSCRIPT) and therefore do not know the variables we will model after preregistration, our model only assumes a continuous variable but it is otherwise free of any other assumptions.
...
Because current pilot data are quite likely unsifficient in terms of power (65 observations), and would hinder testing the design of the statistical models, we will create synthetic data with assumed coefficients for all predictors we plan to add to the model. 
...
```{r dag h1, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r dag h2, message=FALSE, warning=FALSE, echo=TRUE}
...
This is our hypothesis regarding relationship between effort change and answer similarity

*H2: A higher degree of misunderstanding will require a performer to engage in more effortful correction.*

## Contrast coding

Converting collumns to factors
