...
data_feat |> count(modality)

# how many trials per correction level
data_feat |> count(correction)

# And together
data_feat |> count(modality, correction)
...
if (file.exists(here(models, "h1.m4_torq_cum1.rds"))){
  ...
  1 + # Main effects
    ...
    correction:modality +           # primary — does correction effect differ by channel?
    correction:expressibility_z +   # primary — does correction effect depend on concept difficulty?
    modality:expressibility_z +     # covariate — does expressibility-effort relationship differ by modality?
    correction:BFI_extra +          # secondary — do extraverts escalate more at corrections?
    correction:Familiarity +        # secondary — do familiar dyads escalate differently?
    # Random effects
...
pp_check(h1.m3_torq_cum, ndraws = 100)

```

Compute some direct comparisons
...
if (file.exists(here(models, "h1.m4_env_cumi.rds"))){
...
if (file.exists(here(models, "h1.m1_env_in1.rds"))){
...
if (file.exists(here(models, "h1.m4_env_in1.rds"))){
...
if (file.exists(here(models, "h1.m4_cop_cum1.rds"))){
...
if (file.exists(here(models, "h1.m4_cop_in1.rds"))){
...
model_grid <- list(
  list(model = h1.m3_torq_cum, label = "Arm Torque\n(integral)",  row = 1,
       data_modalities = c("gesture", "multimodal"),
       implied_coef    = "multimodal"),
  list(model = h1.m3_env_cum,  label = "Envelope\n(integral)",    row = 1,
       data_modalities = c("vocal", "multimodal"),
       implied_coef    = "multimodal"),
  list(model = h1.m3_cop_cum,  label = "COP\n(integral)",         row = 1,
       data_modalities = c("gesture", "multimodal", "vocal"),
       implied_coef    = "vocal"),
...
ggsave("correction_raincloud_grid.png",
       correction_grid,
       width = 16, height = 10,
       dpi = 300, bg = "white")
...
ggsave("correction_posteriors_grid.png",
       correction_grid,
       width = 12, height = 7,
       dpi = 300, bg = "white")
...
ggsave("fixed_effects_posteriors_grid.png",
       fe_grid, width = 14, height = 8,
       dpi = 300, bg = "white")
...
# Create column TrialNumber by extracting third element from TrialID
df_ges2_final$TrialNumber <- sapply(strsplit(df_ges2_final$TrialID, "_"), function(x) x[3])
df_ges2_final$TrialNumber <- as.numeric(df_ges2_final$TrialNumber)  # Ensure TrialNumber is numeric

# Create sessionID by taking first element
df_ges2_final$SessionID <- sapply(strsplit(df_ges2_final$TrialID, "_"), function(x) x[1])
...
# Create column TrialNumber by extracting third element from TrialID
df_ges3$TrialNumber <- sapply(strsplit(df_ges3$TrialID, "_"), function(x) x[3])
...
# Create sessionID by taking first element
df_ges3$SessionID <- sapply(strsplit(df_ges3$TrialID, "_"), function(x) x[1])
...
# Create column TrialNumber by extracting third element from TrialID
df_voc2_final$TrialNumber <- sapply(strsplit(df_voc2_final$TrialID, "_"), function(x) x[3])
df_voc2_final$TrialNumber <- as.numeric(df_voc2_final$TrialNumber)  # Ensure TrialNumber is numeric

# Create sessionID by taking first element
df_voc2_final$SessionID <- sapply(strsplit(df_voc2_final$TrialID, "_"), function(x) x[1])
...
# Create column TrialNumber by extracting third element from TrialID
df_voc3$TrialNumber <- sapply(strsplit(df_voc3$TrialID, "_"), function(x) x[3])
df_voc3$TrialNumber <- as.numeric(df_voc3$TrialNumber)  # Ensure TrialNumber is numeric

# Center
df_voc3$TrialNumber_c <- df_voc3$TrialNumber - median(range(df_voc3$TrialNumber))

# Create sessionID by taking first element
df_voc3$SessionID <- sapply(strsplit(df_voc3$TrialID, "_"), function(x) x[1])
...
# Create column TrialNumber by extracting third element from TrialID
df_cop2_final$TrialNumber <- sapply(strsplit(df_cop2_final$TrialID, "_"), function(x) x[3])
df_cop2_final$TrialNumber <- as.numeric(df_cop2_final$TrialNumber)  # Ensure TrialNumber is numeric
...
# Create column TrialNumber by extracting third element from TrialID
df_cop3$TrialNumber <- sapply(strsplit(df_cop3$TrialID, "_"), function(x) x[3])
df_cop3$TrialNumber <- as.numeric(df_cop3$TrialNumber)  # Ensure TrialNumber is numeric

# Center
df_cop3$TrialNumber_c <- df_cop3$TrialNumber - median(range(df_cop3$TrialNumber))

# Create sessionID by taking first element
df_cop3$SessionID <- sapply(strsplit(df_cop3$TrialID, "_"), function(x) x[1])
...
dv_labels <- list(
  torq_cum = "Arm Torque (integral)",
  torq_in  = "Arm Torque (peak mean)",
  env_cum  = "Envelope (integral)",
  env_in   = "Envelope (peak mean)",
  cop_cum  = "COP (integral)",
  cop_in   = "COP (peak mean)"
)

plot_h2_prev_answer(models_h2, dv_labels)
...
# ── Create first_trial_effort variable ────────────────────────────────────────
...
if (file.exists(here(models, "e1.m1_torq_in1.rds"))){
...
if (file.exists(here(models, "e1.m1_env_cum1.rds"))){
...
# Create column TrialNumber by extracting third element from TrialID
trials_c0$TrialNumber <- sapply(strsplit(trials_c0$TrialID, "_"), function(x) x[3])
trials_c0$TrialNumber <- as.numeric(trials_c0$TrialNumber)  # Ensure TrialNumber is numeric

# Create sessionID by taking first element
trials_c0$SessionID <- sapply(strsplit(trials_c0$TrialID, "_"), function(x) x[1])
...
### Model 1 - efficiency at c0 w/ modality slope (cumulative)
...
```
### Model 1b - efficiency at c0 (instant)
...
Also, Extract predicted probabilities at each anchor, by modality
And compute slope contrasts
...
ggsave("e4m1_channel_correlations_by_modality.png",
...
# Create column TrialNumber by extracting third element from TrialID
df_eff_rep$TrialNumber <- sapply(strsplit(df_eff_rep$TrialID, "_"), function(x) x[3])
df_eff_rep$TrialNumber <- as.numeric(df_eff_rep$TrialNumber)  # Ensure TrialNumber is numeric

# Create sessionID by taking first element
df_eff_rep$SessionID <- sapply(strsplit(df_eff_rep$TrialID, "_"), function(x) x[1])
...
pp_check(e4.m3b_att, ndraws = 100) # Okay this might be the reason, the posterior distribution is missing the raw data
...
### Model 3 - following success (cumulative)
...
ggsave("e3.m3_att_channel_correlations_by_modality.png",
       result$plot, width = 12, height = 8, dpi = 300)
...
Does overall communicative investment predict how quickly the sequence resolves?
...
df_ordinal <- df_eff_rep |>
  ...
  summarise(
    arm_torque_log_c = mean(arm_torque_log_c, na.rm = TRUE),
...
Does initial communicative investment predict how quickly the sequence resolves?
...
ggsave("e3m5_channel_correlations_by_modality.png",
       result$plot, width = 12, height = 8, dpi = 300)
...
### Model 6 - weighted average (cumulative)

Does the effort invested in the decisive correction attempt predict how quickly the sequence resolves?
...
df_ordinal_weighted <- df_eff_rep |>
  mutate(
    weight = case_when(
      correction == "c0" ~ 1,
      correction == "c1" ~ 2,
  ...
  group_by(pcn_ID, concept, modality) |>
  summarise(
    arm_torque_log_c = weighted.mean(arm_torque_log_c, weight, na.rm = TRUE),
    envelope_log_c   = weighted.mean(envelope_log_c,   weight, na.rm = TRUE),
    copc_log_c       = weighted.mean(copc_log_c,       weight, na.rm = TRUE),
    arm_torque_in_log_c = weighted.mean(arm_torque_in_log_c, weight, na.rm = TRUE),
    envelope_in_log_c   = weighted.mean(envelope_in_log_c,   weight, na.rm = TRUE),
    copc_in_log_c       = weighted.mean(copc_in_log_c,       weight, na.rm = TRUE),
    resolution_phase = first(resolution_phase),
    expressibility_z = first(expressibility_z),
    Familiarity      = first(Familiarity),
    BFI_extra        = first(BFI_extra),
...
contrasts(df_ordinal_weighted$modality) <- contr.treatment(3, base = 2)
...
### Model 7 - resolved yes/no (cumulative)
...
if (file.exists(here(models, "e3.m7_att.rds"))){
  
  e3.m7_att <- readRDS(here(models, "e3.m7_att.rds"))
  e3.m7_att_R2 <- readRDS(here(models, "e3.m7_att_R2.rds"))
  ...
  # Add criterions for later diagnostics
  e3.m7_att <- add_criterion(e3.m7_att, criterion = c("loo", "waic")) 
  
  # Calculate also variance explained (R^2)
  e3.m7_att_R2 <- bayes_R2(e3.m7_att)
  
  # Save both as objects
  saveRDS(e3.m7_att, here(models, "e3.m7_att.rds"))
  saveRDS(e3.m7_att_R2, here(models, "e3.m7_att_R2.rds"))
...
### Model 7b - yes/no (instant)

```{r}

if (file.exists(here(models, "e3.m7b_att.rds"))){
  
  e3.m7b_att <- readRDS(here(models, "e3.m7b_att.rds"))
  e3.m7b_att_R2 <- readRDS(here(models, "e3.m7b_att_R2.rds"))
  ...
  # Add criterions for later diagnostics
  e3.m7b_att <- add_criterion(e3.m7b_att, criterion = c("loo", "waic")) 
  
  # Calculate also variance explained (R^2)
  e3.m7b_att_R2 <- bayes_R2(e3.m7b_att)
  
  # Save both as objects
  saveRDS(e3.m7b_att, here(models, "e3.m7b_att.rds"))
  saveRDS(e3.m7b_att_R2, here(models, "e3.m7b_att_R2.rds"))
...
report_effort_success(
  model       = e3.m7_att,
  data        = df_ordinal_c0,
  model_label = "e3.m7_att",
  output_file = "effort_ever_resolved_e3m7.png"
)
...
ggsave("e3m7_channel_correlations_by_modality.png",
       result$plot, width = 12, height = 8, dpi = 300)
...
report_effort_success(
  ...
  model_label = "e3.m7b_att",
  output_file = "effort_ever_resolved_e3m7b.png"
)
...
### Model 8 - does effort pay-off in the long run
...
df_eff_rep <-
  df_eff_rep |>
  group_by(modality) |>
  mutate(answer_prev_dist_z = (answer_prev_dist - mean(answer_prev_dist, na.rm = T))/ sd(df_eff_rep$answer_prev_dist, na.rm = T)) |>
  ungroup()
...
df_eff_rep <- df_eff_rep %>%
  group_by(SessionID) %>%
  mutate(concept_number = dense_rank(concept),
         concept_number_c = concept_number - median(concept_number))

df_concept_level <- df_eff_rep %>%
  mutate(
    correction = factor(correction, 
                             levels = c("c0", "c1", "c2"), 
                             ordered = TRUE)
  ) %>%
  arrange(SessionID, pcn_ID, concept, correction) %>%
  group_by(SessionID, pcn_ID, concept) %>%
  summarise(
    concept_number        = first(concept_number),
    total_arm_torque_log  = sum(arm_log),
    total_envelope_log    = sum(env_log),
    total_copc_log        = sum(copc_log),
    total_arm_torque_in_log = sum(arm_in_log),
    total_envelope_in_log    = sum(env_in_log),
    total_copc_in_log        = sum(copc_in_log),
    final_similarity      = last(answer_fol_dist),
    answer_prev_dist_z    = first(answer_prev_dist_z[correction == "c1"]),
    expressibility_z      = first(expressibility_z),
  ...
  mutate(
    concept_number_c   = concept_number - median(concept_number),
    arm_torque_total_c = total_arm_torque_log - mean(total_arm_torque_log),
    envelope_total_c   = total_envelope_log   - mean(total_envelope_log),
    copc_total_c       = total_copc_log       - mean(total_copc_log),
    arm_torque_in_total_c = total_arm_torque_in_log - mean(total_arm_torque_in_log),
    envelope_in_total_c   = total_envelope_in_log   - mean(total_envelope_in_log),
    copc_in_total_c       = total_copc_in_log       - mean(total_copc_in_log)
  )

# get rid of trials that have only c0
df_concept_level <- df_concept_level %>%
  filter(!is.na(answer_prev_dist_z))


# NOCHMAL Treatment CC, with multimodal as a baseline
df_concept_level$modality <- factor(df_concept_level$modality,
                          levels = c('gesture', 'multimodal', 'vocal'))
contrasts(df_concept_level$modality) <- contr.treatment(3, base = 2)
...
if (file.exists(here(models, "e3.m8_att.rds"))){
  
  e3.m8_att <- readRDS(here(models, "e3.m8_att.rds"))
  e3.m8_att_R2 <- readRDS(here(models, "e3.m8_att_R2.rds"))
  ...
  # ── Model ─────────────────────────────────────────────────────────────────────
  e3.m8_att <- brm(
    final_similarity ~ arm_torque_total_c * concept_number_c +
                     envelope_total_c   * concept_number_c +
                     copc_total_c       * concept_number_c +
                     expressibility_z   * concept_number_c +
                     answer_prev_dist_z +
                     modality +
                     Familiarity + BFI_extra +
                     (1 + concept_number_c | SessionID) +
                     (1 | pcn_ID) +
                     (1 | concept),
    data   = df_concept_level,
    family = zero_one_inflated_beta(),
    prior  = c(
      prior(normal(0, 0.5), class = "b"),
      prior(normal(0, 1),   class = "Intercept"),
    ...
    init   = 0,          # <-- initialise all parameters at 0 on unconstrained scale
    chains  = 4, cores = 4,
    iter    = 10000, warmup = 4000,
    control = list(adapt_delta = 0.99, max_treedepth = 12),
    seed    = 0209
    ...
    # Add criterions for later diagnostics
  e3.m8_att <- add_criterion(e3.m8_att, criterion = c("loo", "waic")) 
  
  # Calculate also variance explained (R^2)
  e3.m8_att_R2 <- bayes_R2(e3.m8_att)
  
  # Save both as objects
  saveRDS(e3.m8_att, here(models, "e3.m8_att.rds"))
  saveRDS(e3.m8_att_R2, here(models, "e3.m8_att_R2.rds"))
...
if (file.exists(here(models, "e3.m8b_att1.rds"))){
  
  e3.m8b_att <- readRDS(here(models, "e3.m8b_att.rds"))
  e3.m8b_att_R2 <- readRDS(here(models, "e3.m8b_att_R2.rds"))
  ...
  # ── Model ─────────────────────────────────────────────────────────────────────
  e3.m8b_att <- brm(
    final_similarity ~ arm_torque_in_total_c * concept_number_c +
                     envelope_in_total_c   * concept_number_c +
                     copc_in_total_c       * concept_number_c +
                     expressibility_z   * concept_number_c +
                     answer_prev_dist_z +
                     modality +
                     Familiarity + BFI_extra +
    ...
    # Add criterions for later diagnostics
  e3.m8b_att <- add_criterion(e3.m8b_att, criterion = c("loo", "waic")) 
  
  # Calculate also variance explained (R^2)
  e3.m8b_att_R2 <- bayes_R2(e3.m8b_att)
  
  # Save both as objects
  saveRDS(e3.m8b_att, here(models, "e3.m8b_att.rds"))
  saveRDS(e3.m8b_att_R2, here(models, "e3.m8b_att_R2.rds"))
...
dv_labels_m8 <- list(
  arm_torque_total_c    = "Total Upper-limb Effort (cumulative)",
  arm_torque_in_total_c = "Total Upper-limb Effort (instantaneous)",
  copc_total_c          = "Total Postural Effort (cumulative)",
  copc_in_total_c       = "Total Postural Effort (instantaneous)",
  envelope_total_c      = "Total Vocal Effort (cumulative)",
  envelope_in_total_c   = "Total Vocal Effort (instantaneous)"
)

plot_effort_by_concept(model_list_m8, dv_labels_m8)
...
ggsave("e3m8_channel_correlations_by_modality.png",
       result$plot, width = 12, height = 8, dpi = 300)
...
ggsave("e3m8b_channel_correlations_by_modality.png",
       result$plot, width = 12, height = 8, dpi = 300)
