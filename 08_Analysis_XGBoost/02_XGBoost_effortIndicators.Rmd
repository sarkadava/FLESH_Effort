...
tuneGes <- makeClassifTask(data = data_ges[,0:402],
...
gesTuned <- ranger(
  ...
  x = train_data[,0:401], 
  num.trees = 5000, 
  mtry = 94, # Set the recommended mtry value (number of features).
  min.node.size = 2, # Set the recommended min.node.size value (number of samples before a node terminates).
  sample.fraction = 0.7665596, # Set the recommended sample fraction value.(% of data for bagging).
...
tuneGes <- makeClassifTask(data = train_data[, 0:402], target = "correction_info")
...
Now let's collect 3 features per component and bind all to single df
...
top_features_ges <- extract_and_save_top_features(ges_pca, gesCumulativeImportance, n = 3, file_name = "top_features_ges_final.csv")

# Print the resulting dataframe
print(top_features_ges)
...
vocUntuned <- ranger(
  y = train_data$correction_info,
  x = train_data[,0:109], # FLAGGED: adapt
  num.trees = 500,
  importance = "permutation"
)
...
tuneVoc <- makeClassifTask(data = data_voc[,0:110], # FLAGGED: adapt
                           target = "correction_info")
...
vocTuned <- ranger(
  ...
  x = train_data[,0:109],  #FLAGGED: adapt
  num.trees = 5000, 
  mtry = 56, # Set the recommended mtry value (number of features).
  min.node.size = 5, # Set the recommended min.node.size value (number of samples before a node terminates).
  sample.fraction = 0.2063411, # Set the recommended sample fraction value.(% of data for bagging).
  importance = "permutation" # Permutation is a computationally intensive test.
)
...
# Create a classification task for tuning
tuneVoc <- makeClassifTask(data = train_data[, 0:110], target = "correction_info") # FLAGGED: adapt
