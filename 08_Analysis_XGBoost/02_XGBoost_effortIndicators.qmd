...
In the [previous script](../08_Analysis_XGBoost/01_PCA_featureDimensions.ipynb#sec-pca), we have already separated trials into modalities and clean the resulting dataframes of superfluous columns so now we can directly load them and continue modelling.
...
Now, to select features along different (uncorrelated) dimensions, we want to connect these results with the results of PCA we performed in [this script](../08_Analysis_XGBoost/01_PCA_featureDimensions.ipynb#sec-pca).
